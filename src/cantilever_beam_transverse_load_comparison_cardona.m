...
%% CONSTANTS. 
n_elems = [2, 4, 8]; % number of beam elements. 
...
% Cardona Ub Vb values. 
Ub_cardona = [28.986, 30.258, 30.621]; 
Vb_cardona = [65.862, 66.627, 66.874]; 
avg_iterations_cardona = [6.8, 6.9, 6.9]; 
%% SIMULATIONS. 
...
hold on; 
plot(n_elems, Ub_cardona, "-o", color="blue", LineWidth=1); 
xlabel("Number of Beam elements [-]"); 
ylabel("Ub [m]");
title("Ub - horizontal displacement vs No. Beam elements"); 
legend("Ub numerical", "Ub Cardona")
grid; 

figure(2); 
plot(n_elems, Vb_vals,  "-o",color="red", LineWidth=1); 
hold on; 
plot(n_elems, Vb_cardona,  "-o",color="blue", LineWidth=1); 
xlabel("Number of Beam elements [-]"); 
ylabel("Vb [m]");
title("Vb - Vertical displacement vs No. Beam elements"); 
legend(["Vb numerical", "Vb Cardona"]); 
grid; 
